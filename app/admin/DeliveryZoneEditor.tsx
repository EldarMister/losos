...
export function DeliveryZoneEditor({ cityName, points, regionSlug, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    loadMaps().then((ymaps) => {
      ...
      polygon.geometry.events.add("change", () => {
        ...
        setStatus(nextPoints.length >= 3
          ? `${nextPoints.length} точек. Перетаскивайте вершины.`
      ...
      if (points.length >= 3) {
        ...
        void ymaps.geocode(cityName, { results: 1 }).then((result: any) => {
          const coordinates = result.geoObjects.get(0)?.geometry.getCoordinates();
          if (coordinates && !cancelled) map.setCenter(coordinates, 12);
        }).catch(() => undefined);
    ...
    // The map is recreated only when another city is selected.
